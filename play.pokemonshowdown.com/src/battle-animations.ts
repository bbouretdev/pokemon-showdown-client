...
export class PokemonSprite extends Sprite {
	...
	protected static statusTable: {[id: string]: [string, 'good' | 'bad' | 'neutral'] | null | string} = {
		...
		wraithtenacity: ['On Borrow Time', 'good'],
		mindreader: ['True Sight', 'good'],
		lockon: ['True Sight', 'good'],
	};
	forme = '';
	...
	updateStatbar(pokemon: Pokemon, updatePrevhp?: boolean, updateHp?: boolean) {
		...
		} else if (pokemon.status === 'ero') {
			status += '<span class="ero">ERO</span> ';
		} else if (pokemon.status === 'dis') {
			status += '<span class="dis">DIS</span> ';
		}
		if (pokemon.terastallized) {
